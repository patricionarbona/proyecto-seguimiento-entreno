...
export default function CardSaveEjercicio({ datosEjercicio, onClick, setPeso, setSeries, setRepeticiones, setObservacion }) {
      ...
      >
        <div className="absolute h-full w-full rounded-xl shadow-xl [z-index:2] [backface-visibility:hidden] flex flex-col justify-center bg-slate-300 p-0 ">
          ...
          <div>
            ...
            <input id="inputPeso" type="number" onChange={(e) => setPeso(e.target.value)} />
            <label htmlFor="inputSeries">Series</label>
            <input id="inputSeries" type="number" onChange={(e) => setSeries(e.target.value)} />
            <label htmlFor="inputRepeticiones">Repeticiones</label>
            <input id="inputRepeticiones" type="number" onChange={(e) => setRepeticiones(e.target.value)} />
            <label htmlFor="inputObservaciones">Observaciones</label>
            <textarea id="inputObservaciones" onChange={(e) => setObservacion(e.target.value)} />
            <button onClick={onClick} type="button">Añadir</button>
          </div>
        </div>
        <div className="absolute h-full w-full rounded-xl shadow-xl [z-index:2] [backface-visibility:hidden] [transform:rotateY(180deg)] flex flex-col bg-slate-300">
          ...
          <h3>Instrucciones</h3>
          <p className="leading-8 text-ellipsis overflow-hidden h-5/6">{recomendacion}</p>
          <button onClick={onClick} type="button">Hecho</button>
        </div>
      </div>
    </div>
  );
